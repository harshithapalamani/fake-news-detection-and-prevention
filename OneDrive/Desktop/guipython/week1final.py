...
def forgot_password():
    ...
    if username in students_data:
        forgot_question_label.config(text=f"Security Question: {students_data[username]['question']}")
...
def submit_answer():
    ...
    if username in students_data and students_data[username]['answer'] == answer:
        messagebox.showinfo(title="Success", message=f"Your password is: {students_data[username]['password']}")
    else:
        messagebox.showerror(title="Error", message="Incorrect answer.")
