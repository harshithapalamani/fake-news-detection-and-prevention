...
def show_forgot_password_frame():
    ...
    submit_answer_button = tkinter.Button(window, text="Submit Answer", command=submit_answer)
    submit_answer_button.pack(pady=10)

    back_to_login_button = tkinter.Button(window, text="Back to Login", command=switch_to_login)
...
def forgot_password():
    ...
    if username in students_data:
        ...
        forgot_question_label.config(text=f"Security Question: {security_question}")
...
def submit_answer():
    ...
    if username in students_data:
        if students_data[username]['answer'].lower() == answer.lower():
            messagebox.showinfo(title="Success", message="Your password is: " + students_data[username]['password'])
            switch_to_login()
        else:
            messagebox.showerror(title="Error", message="Incorrect answer. Please try again.")
    else:
        messagebox.showerror(title="Error", message="Username not found.")
